...
// Fetch que agafa els districtes de la bbdd
fetch("getDistrictes.php")
.then((response) => response.json())
.then((data) => {
    data.forEach(value => {
        let option = document.createElement("option"); // Creamos un elemento option
        option.value = value.name; // Le ponemos el valor nombre
        option.innerHTML = value.name; // Le ponemos texto 
        select_districtes.appendChild(option); // Lo añadimos al select 
    });
})
...
select_districtes.addEventListener("change", function() {
    ...
    .then((data) => {
        data.forEach(value => {
            ...
            option.value = value.name; // Le ponemos el valor nombre
            option.innerHTML = value.name; // Le ponemos texto 
            select_barris.appendChild(option); // Lo añadimos al select 
        });
    })
})
...
$('#form-user-register').submit(function(e) {
  ...
  //Via Nom Número Pis Escala Porta · CP · Districte · Barri · Pobliacio